...
syms G(t) X(t) I(t) p1 p2 p3 n Gb Ib D_e u_e
syms G_e X_e I_e % Equilibrium variables

% Define the nonlinear equations
eq1 = diff(G(t), t) == -p1 * (G(t) - Gb) - X(t) * G(t) + D_e;
eq2 = diff(X(t), t) == -p2 * X(t) + p3 * (I(t) - Ib);
eq3 = diff(I(t), t) == -n * I(t) + u_e;
...
f1 = -p1 * (G - Gb) - X * G +D_e; % Glucose dynamics
f2 = -p2 * X + p3 * (I - Ib); % Insulin action dynamics
f3 = -n * I+u; % Insulin dynamics
...
Gb_val = 100; % mg/dL
Ib_val = 10; % mU/L
D_e_val = 0; % Glucose disturbance (assume 0 for autonomous system)
u_e_val = 0;

% Substitute the numerical values into the linearized A matrix
A_numeric = double(subs(A_linearized, ...
    [p1, p2, p3, n, Gb, Ib, D_e, u_e], ...
    [p1_val, p2_val, p3_val, n_val, Gb_val, Ib_val,D_e_val, u_e_val]));
